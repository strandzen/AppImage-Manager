...
| `UpdateDaemon` | `updatedaemon.h/.cpp` | Background update checker. Launched via `--daemon` CLI flag (autostart desktop file installs to `$KDE_INSTALL_AUTOSTARTDIR`). Persistent tray entry (`KStatusNotifierItem`) with "Check Now" + "Open Dashboard" actions. Scans `applicationsPath` hourly, uses `GitHubReleaseChecker` + parallelised cold-cache `AppImageReader` reads, fires `KNotification` on update found. Owns a `DownloadWatcher` for `~/Downloads`. Registers D-Bus name `io.github.appimagemanager.Daemon` on start so the dashboard can skip duplicate download notifications. Uses `Qt6::Network` |
...
| `AppImageIconProvider` | `appimageiconprovider.h/.cpp` | `QQuickImageProvider` for `image://appimage/<id>`. Thread-safe via `QReadWriteLock`. Caches rendered pixmaps keyed by `(id, size)`. Key "icon" = manage window single icon; key = `qHash(path)` decimal string = dashboard per-file icon |
...
listModel.checkForUpdates()
  ...
  └─ user confirms → listModel.downloadUpdate(row)
       └─ AppImageUpdateManager::downloadUpdate(...)
            ...
            └─ on success: swap old.new → old, chmod +x, downloadFinished(path, true)
```
